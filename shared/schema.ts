...
// Tabla para almacenar los estados de ánimo de los fans
export const fanMoods = pgTable("fan_moods", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  mood: text("mood", { enum: ["ecstatic", "happy", "hopeful", "neutral", "concerned", "frustrated", "disappointed"] }).notNull(),
  reason: text("reason"),
  relatedToMatch: boolean("related_to_match").default(false),
  matchId: integer("match_id"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const insertFanMoodSchema = createInsertSchema(fanMoods).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertFanMood = z.infer<typeof insertFanMoodSchema>;
export type FanMood = typeof fanMoods.$inferSelect;

// Tabla para almacenar estadísticas agregadas de estados de ánimo
export const moodStats = pgTable("mood_stats", {
  id: serial("id").primaryKey(),
  date: timestamp("date").notNull(),
  totalCount: integer("total_count").notNull(),
  ecstaticCount: integer("ecstatic_count").default(0).notNull(),
  happyCount: integer("happy_count").default(0).notNull(),
  hopefulCount: integer("hopeful_count").default(0).notNull(),
  neutralCount: integer("neutral_count").default(0).notNull(),
  concernedCount: integer("concerned_count").default(0).notNull(),
  frustratedCount: integer("frustrated_count").default(0).notNull(),
  disappointedCount: integer("disappointed_count").default(0).notNull(),
  overallMood: text("overall_mood").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const insertMoodStatSchema = createInsertSchema(moodStats).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertMoodStat = z.infer<typeof insertMoodStatSchema>;
export type MoodStat = typeof moodStats.$inferSelect;
