...
/**
 * API Routes for Fan Mood feature
 */
async function registerFanMoodRoutes(app: Express, storage: IStorage) {
  // Get fan mood stats
  app.get("/api/fan-moods/stats", async (req: Request, res: Response) => {
    try {
      const stats = await storage.getMoodStats();
      res.json(stats);
    } catch (error) {
      console.error("Error retrieving mood stats:", error);
      res.status(500).json({ message: "Error retrieving mood statistics" });
    }
  });

  // Get fan mood trends
  app.get("/api/fan-moods/trends", async (req: Request, res: Response) => {
    try {
      const trends = await storage.getMoodTrends();
      res.json(trends);
    } catch (error) {
      console.error("Error retrieving mood trends:", error);
      res.status(500).json({ message: "Error retrieving mood trends" });
    }
  });

  // Get recent fan moods
  app.get("/api/fan-moods/recent", async (req: Request, res: Response) => {
    try {
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 5;
      const recentMoods = await storage.getRecentFanMoods(limit);
      res.json(recentMoods);
    } catch (error) {
      console.error("Error retrieving recent moods:", error);
      res.status(500).json({ message: "Error retrieving recent moods" });
    }
  });

  // Get user's fan moods
  app.get("/api/fan-moods/user/:userId", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = parseInt(req.params.userId);
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 10;
      
      // Only allow users to access their own moods or admins to access any
      if (req.user && (req.user.id === userId || req.user.role === 'admin')) {
        const userMoods = await storage.getFanMoodsByUserId(userId, limit);
        res.json(userMoods);
      } else {
        res.status(403).json({ message: "Unauthorized access to this user's moods" });
      }
    } catch (error) {
      console.error("Error retrieving user moods:", error);
      res.status(500).json({ message: "Error retrieving user moods" });
    }
  });

  // Get current user's mood
  app.get("/api/fan-moods/current", isAuthenticated, async (req: Request, res: Response) => {
    try {
      if (req.user) {
        const currentMood = await storage.getUserCurrentMood(req.user.id);
      ...
      res.status(500).json({ message: "Error retrieving current mood" });
    }
  });
  ...
  app.post("/api/fan-moods", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const { moodType, reason } = req.body;
      ...
      // Validate mood type
      const validMoodTypes = ['ecstatic', 'happy', 'optimistic', 'neutral', 'concerned', 'frustrated', 'angry'];
      if (!validMoodTypes.includes(moodType)) {
        return res.status(400).json({ message: "Invalid mood type" });
      }

      const mood: InsertFanMood = {
        userId: req.user.id,
        moodType,
        reason: reason || null,
      };

      const newMood = await storage.createFanMood(mood);
      res.status(201).json(newMood);
    } catch (error) {
      console.error("Error creating fan mood:", error);
      res.status(500).json({ message: "Error creating fan mood" });
    }
  });

  // Update fan mood
  app.put("/api/fan-moods/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const moodId = parseInt(req.params.id);
      const { moodType, reason } = req.body;
      
      // Get the current mood to check ownership
      const currentMood = await storage.getFanMoodById(moodId);
      
      if (!currentMood) {
        return res.status(404).json({ message: "Mood not found" });
      }
      
      // Only allow users to update their own moods or admins to update any
      if (req.user && (req.user.id === currentMood.userId || req.user.role === 'admin')) {
        const updatedMood = await storage.updateFanMood(moodId, { moodType, reason });
        res.json(updatedMood);
      } else {
        res.status(403).json({ message: "Unauthorized to update this mood" });
      }
    } catch (error) {
      console.error("Error updating fan mood:", error);
      res.status(500).json({ message: "Error updating fan mood" });
    }
  });

  // Delete fan mood
  app.delete("/api/fan-moods/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const moodId = parseInt(req.params.id);
      ...
      // Only allow users to delete their own moods or admins to delete any
      if (req.user && (req.user.id === currentMood.userId || req.user.role === 'admin')) {
        const success = await storage.deleteFanMood(moodId);
        ...
        res.status(403).json({ message: "Unauthorized to delete this mood" });
      }
    } catch (error) {
      console.error("Error deleting fan mood:", error);
      res.status(500).json({ message: "Error deleting fan mood" });
    }
  });
}
