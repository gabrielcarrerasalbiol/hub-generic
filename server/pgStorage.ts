import { eq, and, desc, like, sql, asc, inArray, count, not, gte, lte } from 'drizzle-orm';
import { db } from './db';
...
import {
  ...
  viewHistory, comments, fanMoods, moodStats
} from '@shared/schema';
...
export class PgStorage implements IStorage {
  ...
  // Fan Mood operations
  async getFanMoods(limit = 100, offset = 0): Promise<FanMood[]> {
    try {
      const result = await db.select().from(fanMoods)
        .limit(limit)
        .offset(offset)
        .orderBy(desc(fanMoods.createdAt));
      return result;
    } catch (error) {
  ...
  async getFanMoodById(id: number): Promise<FanMood | undefined> {
    try {
      const result = await db.select().from(fanMoods)
        .where(eq(fanMoods.id, id))
        .limit(1);
      return result[0];
    } catch (error) {
      console.error(`Error al obtener el estado de ánimo con ID ${id}:`, error);
      return undefined;
    }
  }

  async getFanMoodsByUserId(userId: number, limit = 50): Promise<FanMood[]> {
    try {
      const result = await db.select().from(fanMoods)
        .where(eq(fanMoods.userId, userId))
        .orderBy(desc(fanMoods.createdAt))
        .limit(limit);
      return result;
    } catch (error) {
      console.error(`Error al obtener los estados de ánimo del usuario ${userId}:`, error);
      return [];
    }
  }

  async getRecentFanMoods(limit = 50): Promise<FanMood[]> {
    try {
      const result = await db.select().from(fanMoods)
        .orderBy(desc(fanMoods.createdAt))
        .limit(limit);
  ...
  async getUserCurrentMood(userId: number): Promise<FanMood | undefined> {
    try {
      const result = await db.select().from(fanMoods)
        .where(eq(fanMoods.userId, userId))
        .orderBy(desc(fanMoods.createdAt))
      ...
      console.error(`Error al obtener el estado de ánimo actual del usuario ${userId}:`, error);
      return undefined;
    }
  }

  async createFanMood(mood: InsertFanMood): Promise<FanMood> {
    try {
      const [result] = await db.insert(fanMoods).values(mood).returning();
      
      // Actualizar estadísticas después de crear un nuevo estado de ánimo
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      await this.updateMoodStats(today, mood.mood);
      
      return result;
    } catch (error) {
      console.error('Error al crear un nuevo estado de ánimo:', error);
      throw new Error('Error al crear un nuevo estado de ánimo');
    }
  }

  async updateFanMood(id: number, moodData: Partial<InsertFanMood>): Promise<FanMood | undefined> {
    try {
      const [result] = await db.update(fanMoods)
        .set({
          ...moodData,
          updatedAt: new Date()
        })
      ...
      console.error(`Error al actualizar el estado de ánimo con ID ${id}:`, error);
      return undefined;
    }
  }

  async deleteFanMood(id: number): Promise<boolean> {
    try {
      const result = await db.delete(fanMoods).where(eq(fanMoods.id, id));
      return true;
    } catch (error) {
      console.error(`Error al eliminar el estado de ánimo con ID ${id}:`, error);
      return false;
    }
  }

  // Mood Statistics operations
  async getMoodStats(limit = 30): Promise<MoodStat[]> {
    try {
      const result = await db.select().from(moodStats)
        .orderBy(desc(moodStats.date))
        .limit(limit);
      return result;
    } catch (error) {
      console.error('Error al obtener las estadísticas de estado de ánimo:', error);
      return [];
    }
  }

  async getMoodStatsByDate(date: Date): Promise<MoodStat | undefined> {
    try {
      ...
      const result = await db.select().from(moodStats)
        .where(
          and(
            gte(moodStats.date, startOfDay),
            lte(moodStats.date, endOfDay)
          )
        )
        .limit(1);
      return result[0];
    } catch (error) {
      console.error(`Error al obtener las estadísticas del día ${date.toISOString()}:`, error);
      return undefined;
    }
  }

  async getMoodStatsForDateRange(startDate: Date, endDate: Date): Promise<MoodStat[]> {
    try {
      const result = await db.select().from(moodStats)
        .where(
          and(
            gte(moodStats.date, startDate),
            lte(moodStats.date, endDate)
          )
        )
        .orderBy(asc(moodStats.date));
      return result;
    } catch (error) {
      console.error(`Error al obtener las estadísticas para el rango ${startDate.toISOString()} - ${endDate.toISOString()}:`, error);
      return [];
    }
  }

  async getCurrentDayMoodStats(): Promise<MoodStat | undefined> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return this.getMoodStatsByDate(today);
  }

  async updateMoodStats(date: Date, moodType: string): Promise<MoodStat> {
    try {
      // Normalizar la fecha para comparar solo año, mes y día
      const normalizedDate = new Date(date);
      normalizedDate.setHours(0, 0, 0, 0);

      // Buscar si ya existen estadísticas para esta fecha
      const existingStats = await this.getMoodStatsByDate(normalizedDate);
      ...
      if (existingStats) {
        ...
        // Incrementar el contador específico según el tipo de estado de ánimo
        switch (moodType) {
          case 'ecstatic':
            updates.ecstaticCount = existingStats.ecstaticCount + 1;
            break;
          case 'happy':
            updates.happyCount = existingStats.happyCount + 1;
            break;
          case 'hopeful':
            updates.hopefulCount = existingStats.hopefulCount + 1;
            break;
          case 'neutral':
            updates.neutralCount = existingStats.neutralCount + 1;
            break;
          case 'concerned':
            updates.concernedCount = existingStats.concernedCount + 1;
            break;
          case 'frustrated':
            updates.frustratedCount = existingStats.frustratedCount + 1;
            break;
          case 'disappointed':
            updates.disappointedCount = existingStats.disappointedCount + 1;
            break;
        }

        // Calcular el estado de ánimo general
        const updatedStats = { ...existingStats, ...updates };
        updates.overallMood = this.calculateOverallMood(updatedStats);

        // Actualizar en la base de datos
        const [result] = await db.update(moodStats)
          .set(updates)
          .where(eq(moodStats.id, existingStats.id))
          .returning();
        
        return result;
      } else {
        // Crear nuevas estadísticas para esta fecha
        const newStats: Partial<InsertMoodStat> = {
          date: normalizedDate,
          totalCount: 1,
          ecstaticCount: 0,
          happyCount: 0,
          ...
          frustratedCount: 0,
          disappointedCount: 0,
          overallMood: 'neutral' // Valor predeterminado
        };

        // Incrementar el contador específico según el tipo de estado de ánimo
        switch (moodType) {
          case 'ecstatic':
            newStats.ecstaticCount = 1;
            break;
          case 'happy':
            newStats.happyCount = 1;
            break;
          case 'hopeful':
            newStats.hopefulCount = 1;
            break;
          case 'neutral':
            newStats.neutralCount = 1;
            break;
          case 'concerned':
            newStats.concernedCount = 1;
            break;
          case 'frustrated':
            newStats.frustratedCount = 1;
            break;
          case 'disappointed':
            newStats.disappointedCount = 1;
            break;
        }

        // Calcular el estado de ánimo general
        newStats.overallMood = moodType;

        // Insertar en la base de datos
        const [result] = await db.insert(moodStats).values(newStats as InsertMoodStat).returning();
        return result;
      }
    } catch (error) {
      console.error(`Error al actualizar las estadísticas de estado de ánimo:`, error);
      throw new Error('Error al actualizar las estadísticas de estado de ánimo');
    }
  }

  async getMoodTrend(days = 7): Promise<{date: string, overallMood: string, totalCount: number}[]> {
    try {
      ...
      const stats = await this.getMoodStatsForDateRange(startDate, endDate);
      
      // Transformar los resultados al formato deseado
      return stats.map(stat => ({
        date: stat.date.toISOString().split('T')[0], // Formato YYYY-MM-DD
        overallMood: stat.overallMood,
        totalCount: stat.totalCount
      }));
    } catch (error) {
      console.error(`Error al obtener la tendencia de estado de ánimo:`, error);
      return [];
    }
  }

  // Método auxiliar para calcular el estado de ánimo general
  private calculateOverallMood(stats: MoodStat): string {
    // Determinar qué estado de ánimo tiene más votos
    const moodCounts = [
      { mood: 'ecstatic', count: stats.ecstaticCount },
      { mood: 'happy', count: stats.happyCount },
      { mood: 'hopeful', count: stats.hopefulCount },
      { mood: 'neutral', count: stats.neutralCount },
      { mood: 'concerned', count: stats.concernedCount },
      { mood: 'frustrated', count: stats.frustratedCount },
      { mood: 'disappointed', count: stats.disappointedCount }
    ];

    moodCounts.sort((a, b) => b.count - a.count);
    
    if (moodCounts[0].count === 0) {
      return 'neutral'; // Valor predeterminado si no hay votos
    }
    
    return moodCounts[0].mood;
  }
}
