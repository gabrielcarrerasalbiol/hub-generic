...
import { Search, ArrowLeft, Crown, Lock } from "lucide-react";

export default function PremiumChannelsPage() {
  const { t } = useLanguage();
  const [searchQuery, setSearchQuery] = useState("");
  const isPremium = useAuth(state => state.isPremium());
  
  // Fetch premium channels
  const { data: premiumChannels = [], isLoading } = useQuery<Channel[]>({
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="relative mb-8 max-w-lg">
        ...
        <Input
          type="text"
          placeholder="Buscar canales premium..."
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          {searchQuery && (
            ...
            >
              Mostrar todos los canales premium
            </Button>
          )}
        </div>
      )}
